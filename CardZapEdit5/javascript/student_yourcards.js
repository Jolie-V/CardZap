document.addEventListener("DOMContentLoaded", () => {
  ...
  createDeckBtn.addEventListener('click', () => {
    ...
    const editingDeckId = cardModal.dataset.editingDeckId;
    
    if (editingDeckId) {
      // Update existing deck
      const existingDeckCard = document.querySelector(`[data-deck-id="${editingDeckId}"]`);
      if (existingDeckCard) {
        updateDeckCard(existingDeckCard, deckData);
        console.log('Updated deck:', deckData);
      }
    } else {
      // Create new deck
  ...
  function createDeckCard(deckData) {
    ...
    // Add right-click context menu (will be re-attached by navigation handler too)
    ...
    // Attach navigation behavior based on questioning type (Classic or Quiz)
    attachNavigationBehavior(deckCard, deckData);
  }
  ...
  // Attach click navigation behavior based on questioning type, and re-attach context menu
  function attachNavigationBehavior(deckCard, deckData) {
    const parent = deckCard.parentNode;
    if (!parent) return;
    
    const newDeckCard = deckCard.cloneNode(true);
    
    // Default cursor
    newDeckCard.style.cursor = '';
    ...
    if (deckData.questioningType === 'Classic') {
      newDeckCard.style.cursor = 'pointer';
      newDeckCard.addEventListener('click', () => {
        const params = buildParams();
        window.location.href = `student_classiccards.php?${params.toString()}`;
      });
    } else if (deckData.questioningType === 'Quiz') {
      newDeckCard.style.cursor = 'pointer';
      newDeckCard.addEventListener('click', () => {
        const params = buildParams();
        window.location.href = `student_quizcards.php?${params.toString()}`;
      });
    }
    
    parent.replaceChild(newDeckCard, deckCard);
  }
  ...
  function resetForm() {
    ...
    // Reset editing state
    delete cardModal.dataset.editingDeckId;
  ...
  function editDeck(deckCard, deckData) {
    ...
    // Store the deck card being edited
    cardModal.dataset.editingDeckId = deckCard.dataset.deckId;
    
    // Modify create button to update existing deck
    createDeckBtn.textContent = 'Update Deck';
    
    // Store original create button functionality
    if (!createDeckBtn.dataset.originalOnclick) {
      createDeckBtn.dataset.originalOnclick = 'true';
      createDeckBtn.dataset.originalText = 'Create';
    }
  }
  ...
  function updateDeckCard(deckCard, deckData) {
    ...
    // Update click functionality (Classic or Quiz)
    attachNavigationBehavior(deckCard, deckData);
  }
