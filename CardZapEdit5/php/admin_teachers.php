      ...
      <main class="content">
        ...
        <div class="search-row" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
          <form id="searchForm" method="get" style="display: flex; gap: 8px; flex: 1;">
            <input class="search-input" type="text" name="q" placeholder="🔍 Search teachers..." value="<?php echo isset($_GET['q']) ? htmlspecialchars($_GET['q']) : ''; ?>" style="padding: 10px 14px; border-radius: 6px; border: 1px solid #d0d7de; width: 100%;"/>
          ...
          <table style="width: 100%; border-collapse: collapse;">
            <thead>
              <tr style="background: #f6f8fa;">
                ...
                <th class="small" style="padding: 12px 8px; text-align: left;">EMAIL</th>
            ...
            <tbody>
              ...
              if ($conn->connect_error) {
                ...
                $sql = "SELECT user_info_id, full_name, e_mail FROM user_info WHERE user_type = 'T'";
                if ($search !== '') {
                  $sql .= " AND (full_name LIKE '%$search%' OR e_mail LIKE '%$search%')";
                }
                ...
                if ($result && $result->num_rows > 0) {
                  while ($row = $result->fetch_assoc()) {
                    ...
                    echo "<td class='small' style='padding: 12px 8px;'>" . htmlspecialchars($row['e_mail']) . "</td>";
                    echo "<td class='small' style='padding: 12px 8px;'>
                      <a href='edit_user.php?id=" . $row['user_info_id'] . "' class='btn btn-sm btn-primary' style='margin-right: 8px;'>Edit</a>
                      <a href='delete_user.php?id=" . $row['user_info_id'] . "' class='btn btn-sm btn-danger' style='background: #e57373; border: none;' onclick=\"return confirm('Delete this teacher?');\">Delete</a>
                    </td>";
                    echo "</tr>";
                  }
                } else {
                  echo '<tr><td colspan="3" style="text-align:center; color: #888; padding: 24px 0;">No teachers found.</td></tr>';
                }
                $conn->close();
              }
              ?>
