...
function onDragMove(x, y) {
  ...
  // Apply translate with slight tilt, preserving flip orientation
  const rotateY = isFlipped ? ' rotateY(180deg)' : '';
  const tilt = Math.max(-10, Math.min(10, dx / 10));
  flashcard.style.transform = `translateX(${dx}px) rotate(${tilt}deg)${rotateY}`;
}
...
function onDragEnd(x, y) {
  ...
  if (absDx >= COMMIT_THRESHOLD_PX && absDy < 120) {
    ...
    flashcard.style.transform = `translateX(${offX}px) rotate(${dx > 0 ? 12 : -12}deg)${rotateY}`;
    setTimeout(() => {
      resetCardTransform();
      nextCard();
    }, 200);
    ...
    flashcard.style.transition = 'transform 0.25s ease';
    const rotateY = isFlipped ? ' rotateY(180deg)' : '';
    flashcard.style.transform = `translateX(0px) rotate(0deg)${rotateY}`;
